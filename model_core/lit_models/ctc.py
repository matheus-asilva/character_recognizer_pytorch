...
def compute_input_lengths(padded_sequences: torch.Tensor) -> torch.Tensor:
    """
    Args:
        padded_sequences (tuple): (N, S) tensors where elements that equal 0 correspond to padding

    Returns:
        torch.Tensor: (N,) tensor where each element corresponds to the non-padded length of each sequence
...
class CTCLitModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        logits = self(x)
        logsprobs = torch.log_softmax(logits, dim=1)
        B, _C, S = logprobs.shape
        ...
        loss = self.loss_fn(logprobs_for_loss, y, input_inputs, target_lengths)
        self.log("val_loss", loss, prog_bar=True)

        decoded = self.greedy_decode(logprobs, max_length=y.shap[1])
    ...
    def greedy_decode(self, logprobs: torch.Tensor, max_length: int) -> torch.Tensor:
        ...
        Args:
            logprobs (tuple): (B, C, S) log probabilities
        ...
        for i in range(B):
            seq = [b for b, _g in itertools.groupby(argmax[i].tolist()) if b != self.blank_index][:max_legnth]
