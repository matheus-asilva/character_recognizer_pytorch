...
from model_core.data.utils import BaseDataset
...
DOWNLOADED_DATA_DIRNAME = BaseDataModule.data_dirname() / "downloaded" / "emnist"
...
class EMNIST(BaseDataModule):
    ...
    def __init__(self, args=None):
        ...
        with open(ESSENTIALS_FILENAME) as file:
            essentials = json.load(file)
        self.mapping = list(essentials["characters"])
        self.inverse_mapping = {value: key for key, value in enumerate(self.mapping)}
    ...
    def setup(self, stage: str = None):
        if stage == "fit" or stage is None:
            with h5py.File(PROCESSED_DATA_FILENAME, "r") as file:
                self.x_trainval = file["x_train"][:]
                self.y_trainval = file["y_train"][:].squeeze().astype(int)
        ...
        if stage == "test" or stage is None:
            with h5py.File(PROCESSED_DATA_FILENAME, "r") as file:
                ...
                self.y_test = file["y_test"][:].squeeze().astype(int)
...
def _download_and_process_emnist():
    ...
    _download_raw_dataset(metadata, DOWNLOADED_DATA_DIRNAME)
    _process_raw_dataset(metadata["filename"], DOWNLOADED_DATA_DIRNAME)
...
def _process_raw_dataset(filename: str, dirname: Path):
    ...
    with h5py.File(PROCESSED_DATA_FILENAME, "w") as file:
        file.create_dataset("x_train", data=x_train, dtype="u1", compression="lzf")
        file.create_dataset("y_train", data=y_train, dtype="u1", compression="lzf")
        file.create_dataset("x_test", data=x_test, dtype="u1", compression="lzf")
        file.create_dataset("y_test", data=y_test, dtype="u1", compression="lzf")
